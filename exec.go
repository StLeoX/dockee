...
// nsenter里的C代码里已经出现dockee_pid和dockee_cmd这两个Key,主要是为了控制是否执行C代码里面的setns.
const (
	EnvExecPid = "dockee_pid"
	EnvExecCmd = "dockee_cmd"
)
...
func getEnvsByPid(pid string) []string {
	envi := fmt.Sprintf("/proc/%s/environ", pid)
	contentBytes, err := os.ReadFile(envi)
	if err != nil {
		log.Errorf("Read file %s error %v", envi, err)
		return nil
	}
	// env split by \u0000
