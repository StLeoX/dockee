...
var runCommand = cli.Command{
	...
	Usage: `Create a container with namespace and cgroups limit
			dockee run -it [command]
			dockee run -d -name [containerName] [imageName] [command]`,
	Flags: []cli.Flag{
		cli.BoolFlag{
		...
		cli.StringSliceFlag{
			Name:  "e",
			Usage: "set environment,e.g. -e name=dockee",
		},
		cli.StringFlag{
			Name:  "net",
...
var commitCommand = cli.Command{
	Name:  "commit",
	Usage: "commit container to image,e.g. dockee commit 123456789 myimage",
	Action: func(context *cli.Context) error {
...
var execCommand = cli.Command{
	Name:  "exec",
	Usage: "exec a command into container,dockee exec 123456789 /bin/sh",
	Action: func(context *cli.Context) error {
		...
		// 格式：dockee exec 容器名字 命令，因此至少会有两个参数
		if len(context.Args()) < 2 {
...
var stopCommand = cli.Command{
	Name:  "stop",
	Usage: "stop a container,e.g. dockee stop 1234567890",
	Action: func(context *cli.Context) error {
		// 期望输入是：dockee stop 容器Id，如果没有指定参数直接打印错误
		if len(context.Args()) < 1 {
...
var removeCommand = cli.Command{
	Name:  "rm",
	Usage: "remove unused containers,e.g. dockee rm 1234567890",
	Flags: []cli.Flag{
		cli.BoolFlag{
			Name:  "f", // 强制删除
